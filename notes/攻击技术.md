...
让被攻击网站执行恶意脚本，实现跳转到新网站、获取cookie等
...
### 2. 过滤特殊字符(别人放的脚本不能运行，正常html可以使用)
...
XSS被攻击网站执行了恶意脚本，CSRF访问被攻击网站的用户是不是“知情“用户
...
Java 中的 PreparedStatement 是**预先编译的 SQL 语句**，可以传入适当参数并且多次执行。由于没有**拼接**的过程，因此可以防止 SQL 注入的发生。
